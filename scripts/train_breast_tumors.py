...
def main() -> None:
    parser = argparse.ArgumentParser("Train on MedCLIP-SAMv2 breast_tumors")
    parser.add_argument("--data-root", type=str, default=str(TEXTFAENET_ROOT.parent / "dataset" / "MedCLIP-SAMv2_data" / "breast_tumors"))
    parser.add_argument("--save-dir", type=str, default=str(TEXTFAENET_ROOT / "runs" / "breast_tumors"))
    ...
    # Standard v6 protocol: train on train_ds, monitor on val_ds, evaluate on test_ds.
    ...
    # epochs is treated as the TOTAL target epoch count, not "how many more to run".
    # This makes resume safe: same --epochs 80 flag works on fresh runs and resumes.
    end_epoch = args.epochs
    total_phase_epochs = max(args.epochs, 1)

    warmup_epochs = max(0, int(getattr(args, "lr_warmup_epochs", 0)))
    for epoch in range(start_epoch, end_epoch + 1):
        phase_epoch = epoch - 1  # absolute, so LR schedule continues correctly on resume
