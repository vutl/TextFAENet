# Missing paper experiments runbook

## Current Table 2--5 results

The block below is regenerated by
`scripts/collect_paper_tables_2_5_dual_metrics.py`. Values are percentages.

<!-- TABLES_2_5_RESULTS_START -->
...
<!-- TABLES_2_5_RESULTS_END -->

## One-command run

From `D:\Documents\LMIS\Text-FAENet`:

```powershell
$env:HF_HOME=(Resolve-Path .hf_cache).Path
$env:TORCH_HOME=(Resolve-Path .torch_cache).Path
D:\anaconda3\python.exe -u scripts\run_paper_missing_experiments.py --run-prefix paper_missing0715
```

The runner is sequential, logs live output, continues after a failed task, and
is safe to invoke again:

- MedCLIP saliency generation skips maps already present.
- Completed MedCLIP tasks are skipped when `summary.json` exists.
- FMISeg uses `resume_latest: true` and resumes its newest checkpoint.
- The QaTa plan uses `--resume-existing --skip-completed` per variant.
- The Table 2--5 metric ledger is regenerated at the end.

Combined logs are written under `runs/batch_logs/`. Failures are summarized in
`runs/batch_logs/paper_missing0715_failures.json`.

## Tasks executed

1. MedCLIP-SAMv2 zero-shot inference with our structured test prompts:
   Brain (600), Breast (113), QaTa (2113), and MosMed (273).
2. FMISeg supervised runs:
   Brain structured, MosMed structured, Brain MedCLIP-style, and Breast
   MedCLIP-style prompts.
3. QaTa ResNet50 lightweight-text decoder retraining:
   `w/o LL`, `w/o LH`, `w/o HL`, and `w/o HH`.
4. Full-test dual-metric inference for all four new drop-band checkpoints.
5. A protocol-matched full-band main reference (`LL/LH/HL/HH`, no dropped band).
6. Regeneration of `paper_tables_2_5_dual_metrics_20260715.{md,json}`.

## Run only one group

```powershell
D:\anaconda3\python.exe -u scripts\run_paper_missing_experiments.py `
  --run-prefix paper_missing0715 `
  --tasks qata_drop_bands
```
...
- MedCLIP dual metrics: `external_metrics/medclipsamv2_*_structured/summary.json`
- FMISeg dual metrics: `external_runs/fmiseg_*/final_test_metrics.json`
- Drop-band dual metrics: `paper_missing0715_drop_band_dual_metrics.{md,json}`
- Unified current ledger: `paper_tables_2_5_dual_metrics_20260715.md`
- Machine-readable ledger: `paper_tables_2_5_dual_metrics_20260715.json`

## Protocol safeguards

- MedCLIP structured runs save the exact prompt map in `prompts_used.json`.
- Brain/Breast use their Text-FAENet structured test CSVs.
- QaTa uses `QaTa-COV19-v2/prompt/test.csv`.
- MosMed uses all 273 rows from `Test_text_MosMedData+(in).csv`, not the
  unrelated 1800-image legacy `lung_CT` split.
- FMISeg checkpoint selection monitors validation mean per-image Dice.
- Every new local result exports both mean per-image and pixel-pooled global
  Dice/IoU.

Expect substantial storage use because FMISeg retains best and last
checkpoints and each is roughly 1.5 GB. Keep at least 20 GB free for all tasks
and intermediate MedCLIP masks.
